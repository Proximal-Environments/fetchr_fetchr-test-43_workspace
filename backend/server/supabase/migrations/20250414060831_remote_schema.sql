...
create policy "Allow users, admins, and stylists to access photos"
on "storage"."objects"
as permissive
for select
to public
using (((bucket_id = 'user_onboarding_photos'::text) AND (((auth.uid())::text = (storage.foldername(name))[1]) OR (EXISTS ( SELECT 1
   FROM users
  WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['ADMIN'::user_role, 'STYLIST'::user_role]))))))));
...
for delete
to authenticated
using ((bucket_id = 'product_photos'::text));
...
for insert
to authenticated
with check ((bucket_id = 'product_photos'::text));


create policy "Give users access to own folder oxt5td_1"
on "storage"."objects"
as permissive
for insert
to public
with check (((bucket_id = 'user_onboarding_photos'::text) AND (( SELECT (auth.uid())::text AS uid) = (storage.foldername(name))[1])));


create policy "Give users access to own folder oxt5td_2"
on "storage"."objects"
as permissive
for update
to public
using (((bucket_id = 'user_onboarding_photos'::text) AND (( SELECT (auth.uid())::text AS uid) = (storage.foldername(name))[1])));


create policy "Give users access to own folder xhwjew_0"
on "storage"."objects"
as permissive
for select
to public
using (((bucket_id = 'chat_photos'::text) AND (( SELECT (auth.uid())::text AS uid) = (storage.foldername(name))[1])));


create policy "Give users access to own folder xhwjew_1"
on "storage"."objects"
as permissive
for insert
to public
with check (((bucket_id = 'chat_photos'::text) AND (( SELECT (auth.uid())::text AS uid) = (storage.foldername(name))[1])));


create policy "Give users access to own folder xhwjew_2"
on "storage"."objects"
as permissive
for update
to public
